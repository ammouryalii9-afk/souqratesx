...
- Monetization is fully wired end-to-end and auto-activates once the admin pastes real keys into Admin Settings — no further code changes needed per provider:
  ...
  - Onclicka (rewarded ad for Telegram Mini Apps): same pattern as Monetag — `onclickaSpotId` in Admin Settings (the numeric `data-admpid` from an Onclicka **Telegram Mini App** ad unit, NOT the obfuscated website adManager tag), frontend loads `js.onclckmn.com/static/onclicka.js` + `initCdTma({id})` in `artifacts/vaultx/src/lib/onclicka.ts`, claims via `POST /earn/onclicka/reward`, shares the same ads cooldown/daily-cap columns. Ad fallback chain is adsgram → monetag → onclicka on no-fill (`adFallback.ts`).
