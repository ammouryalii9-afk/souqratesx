...
- Two-rate claimed economy (2026-07-13): NOTHING is withdrawable until the player presses Claim. All earnings fill the Mined buffer (`state.tempMiningPoints`, with `state.adMiningPoints` tracking the ad-earned portion). `POST /vault/claim` converts server-side in one atomic UPDATE: ad portion at 100% + game remainder at admin `gameToSpendablePercent`% (default 0) into `state.claimedPoints` — the ONLY withdrawable balance (`claimedPoints - withdrawnPoints`). `lifetimePoints` is leaderboard-only, never withdrawable. `claimedPoints` and `claimSeq` are in `PROTECTED_STATE_KEYS`; `claimSeq` is bumped by every claim, and `PUT /vault/me`'s UPDATE is guarded on it (claimSeq unchanged since pre-read) — a stale in-flight sync racing a claim is dropped (server returns current row; client's running totals re-apply on the next debounced sync). Any new code path that rewrites the whole state blob must respect the same guard or claims can be reverted/double-claimed.
