...
/**
 * Onclicka rewarded-ad provider for Telegram Mini Apps (SDK-based, like
 * Monetag — the client SDK's show() Promise only resolves after a completed
 * view). Shares the same `vault_users` ads* cooldown/daily-cap columns as
 * Adsgram/Monetag so a user's total rewarded-ad volume across networks is
 * capped together.
 */
export function createOnclickaProvider(): EarnProvider {
  let config: OnclickaConfig = { spotId: "", rewardPoints: 100, cooldownSeconds: 30, dailyCap: 20 };
  let enabled = false;
  ...
  return {
    ...
    initialize(rawConfig) {
      ...
      enabled = config.spotId.length > 0;
    },
    ...
    async getOffers(_ctx: ProviderContext): Promise<EarnOffer[]> {
      ...
      return [
        {
          providerKey: "onclicka",
          type: "rewarded_ad",
          title: "\u0634\u0627\u0647\u062F \u0625\u0639\u0644\u0627\u0646 Onclicka",
    ...
    async verifyReward(input: RewardVerifyInput) {
      // Client-triggered only: the Onclicka TMA SDK's show() promise only
      // resolves after a completed ad view, trusted the same way as
      // Adsgram/Monetag client-triggered claims (bounded by cooldown + cap).
