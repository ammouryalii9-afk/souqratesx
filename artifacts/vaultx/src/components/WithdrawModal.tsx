      ...
      >
        ...
        <div className="px-5 pt-5 pb-3 flex items-center justify-between flex-shrink-0 border-b border-white/5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Convert your SKX to TON</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 flex flex-col gap-5">
          ...
          >
            <span className="text-xs text-muted-foreground">Available Balance (SKX)</span>
            <span className="text-sm font-bold text-primary">{Math.floor(availablePoints).toLocaleString()} SKX</span>
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                {points > 0 && tonPrice && (
                  <div className="mt-2 flex items-center justify-between px-3 py-2 rounded-lg bg-white/3">
                    <span className="text-xs text-muted-foreground">{points.toLocaleString()} SKX →</span>
                ...
                {points > 0 && !meetsMinimum && (
                  <p className="text-xs text-amber-400 mt-1.5 flex items-center gap-1">
                    ...
                    Minimum is {minPoints.toLocaleString()} SKX (${MIN_WITHDRAWAL_USD.toFixed(2)})
                  </p>
                )}
                {points > 0 && !hasEnough && (
                  <p className="text-xs text-red-400 mt-1.5 flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" />
                    Exceeds your SKX balance
                  </p>
                )}
              </div>
              ...
              >
                ...
                <p className="text-amber-200/70 leading-relaxed">
                  SKX is deducted immediately when you submit. If your request is rejected, the SKX is refunded to your account.
                </p>
              </div>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.pointsAmount.toLocaleString()} SKX · ${item.usdAmount.toFixed(3)} · @${item.tonPriceUsd.toFixed(2)}/TON
                      </p>
        ...
        {/* SKP balance info (spendable on upgrades / convertible — not withdrawable) */}
        <div className="px-5 py-3 border-t border-white/5 flex items-center justify-between flex-shrink-0">
          <span className="text-xs text-muted-foreground">SKP (Mined) Balance</span>
