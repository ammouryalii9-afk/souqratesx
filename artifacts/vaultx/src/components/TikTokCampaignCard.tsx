...
function ApplyModal({ campaign, onClose, onSuccess }: ApplyModalProps) {
  ...
  async function submit() {
    if (!tiktokUsername.trim() || !videoUrl.trim()) { setError("يرجى ملء جميع الحقول"); return; }
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={onClose}>
      <div style={{ width:"100%", maxWidth:480, background:"#121212", borderRadius:"20px 20px 0 0", padding:"24px 20px 36px", border:"1px solid rgba(255,255,255,0.1)" }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ margin:"0 0 4px", fontSize:16, fontWeight:900, color:"#fff" }}>التقديم في {campaign.title}</p>
        <p style={{ margin:"0 0 20px", fontSize:12, color:"rgba(255,255,255,0.45)" }}>ادخل معلوماتك على TikTok وسيتم مراجعة طلبك خلال 24 ساعة</p>

        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div>
            <p style={{ margin:"0 0 6px", fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.6)" }}>اسم حسابك على TikTok</p>
            <input
          ...
          <div>
            <p style={{ margin:"0 0 6px", fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.6)" }}>رابط الفيديو على TikTok</p>
            <input
              value={videoUrl}
              ...
              style={{ width:"100%", boxSizing:"border-box", padding:"10px 14px", borderRadius:10, background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", color:"#fff", fontSize:14, outline:"none" }}
            />
            <p style={{ margin:"4px 0 0", fontSize:11, color:"rgba(255,255,255,0.3)" }}>يجب أن يحصل الفيديو على {campaign.minViews.toLocaleString()}+ مشاهدة</p>
          </div>
        </div>
  ...
  function StatusBadge({ app }: { app: MyApplication }) {
    if (app.status === "approved") return (
      <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, fontWeight:700, color:"#86efac", background:"rgba(34,197,94,0.15)", border:"1px solid rgba(34,197,94,0.3)", borderRadius:6, padding:"3px 8px" }}>
        <CheckCircle style={{ width:11, height:11 }} /> مقبول
      </span>
    );
    if (app.status === "rejected") return (
      <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, fontWeight:700, color:"#f87171", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.3)", borderRadius:6, padding:"3px 8px" }}>
        <XCircle style={{ width:11, height:11 }} /> مرفوض
      </span>
    );
    return (
      <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, fontWeight:700, color:"#fbbf24", background:"rgba(251,191,36,0.1)", border:"1px solid rgba(251,191,36,0.3)", borderRadius:6, padding:"3px 8px" }}>
        <Clock style={{ width:11, height:11 }} /> قيد المراجعة
      </span>
    );
  }
    ...
    <>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:2 }}>
          ...
          <p style={{ margin:0, fontSize:13, fontWeight:900, color:"#fff" }}>مسابقات TikTok</p>
          <span style={{ fontSize:10, fontWeight:700, color:"#e879f9", background:"rgba(232,121,249,0.15)", border:"1px solid rgba(232,121,249,0.3)", borderRadius:5, padding:"2px 7px" }}>حصري</span>
        </div>
            ...
            <div key={c.id} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.09)", borderRadius:16, padding:"14px 16px", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
                <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:10, padding:"8px 10px", display:"flex", alignItems:"center", gap:6 }}>
                  ...
                  <div>
                    <p style={{ margin:0, fontSize:10, color:"rgba(255,255,255,0.4)", fontWeight:600 }}>متابعون</p>
                ...
                <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:10, padding:"8px 10px", display:"flex", alignItems:"center", gap:6 }}>
                  ...
                  <div>
                    <p style={{ margin:0, fontSize:10, color:"rgba(255,255,255,0.4)", fontWeight:600 }}>مشاهدات</p>
              ...
              <div style={{ background:"linear-gradient(135deg,rgba(232,121,249,0.08),rgba(168,85,247,0.08))", border:"1px solid rgba(232,121,249,0.2)", borderRadius:10, padding:"10px 12px", marginBottom:12 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    <p style={{ margin:"0 0 1px", fontSize:10, color:"rgba(255,255,255,0.45)", fontWeight:600 }}>جائزة القبول الفورية</p>
                    <p style={{ margin:0, fontSize:18, fontWeight:900, color:"#e879f9" }}>{fmt(c.prizeSkx)} SKX</p>
                  </div>
                  {c.perReferralSkx > 0 && (
                    <div style={{ textAlign:"right" }}>
                      <p style={{ margin:"0 0 1px", fontSize:10, color:"rgba(255,255,255,0.45)", fontWeight:600 }}>+ لكل إحالة</p>
              ...
              {myApp?.status === "approved" && (
                <div style={{ background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:10, padding:"8px 12px", marginBottom:12, display:"flex", justifyContent:"space-between" }}>
                  <div><p style={{ margin:"0 0 1px", fontSize:10, color:"rgba(255,255,255,0.4)" }}>إجمالي مكتسب</p><p style={{ margin:0, fontSize:13, fontWeight:800, color:"#86efac" }}>{fmt((myApp.rewardSkxPaid ?? 0) + (myApp.totalReferralSkx ?? 0))} SKX</p></div>
                  <div style={{ textAlign:"right" }}><p style={{ margin:"0 0 1px", fontSize:10, color:"rgba(255,255,255,0.4)" }}>إحالات</p><p style={{ margin:0, fontSize:13, fontWeight:800, color:"#86efac" }}>{fmt(myApp.referralCount ?? 0)}</p></div>
                </div>
              )}
              ...
              {myApp?.status === "rejected" && myApp.rejectReason && (
                <div style={{ background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:10, padding:"8px 12px", marginBottom:12 }}>
                  <p style={{ margin:0, fontSize:11, color:"#f87171" }}>السبب: {myApp.rejectReason}</p>
                </div>
              )}
              ...
              {!myApp ? (
                ...
                >
                  <Send style={{ width:14, height:14 }} /> اشترك الآن
                </button>
              ) : myApp.status === "approved" ? (
                <a
                  href={`https://t.me/share/url?url=${encodeURIComponent(window.location.origin)}&text=${encodeURIComponent("انضم لـ SouqratesX واكسب SKX 🚀")}`}
                ...
                >
                  <ExternalLink style={{ width:13, height:13 }} /> شارك رابطك وزِد أرباحك
                </a>
              ) : null}
            </div>
          );
